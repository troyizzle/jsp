class TopicsController < ApplicationController
  ...
  def show
    ...
    @pagy, @posts = pagy(@topic.posts.order(:created_at), items: current_user.try(:posts_per_page) || 10)

    @post = Post.new if current_user
  end
  ...
  def topic_params
    params.require(:topic).permit(:forum_id, :title)
  end
end
