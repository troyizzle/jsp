class Shadcn::SelectComponent
  ...
  def initialize(name:, view_context:, choices: nil, selected: nil, **options, &block)
    @name = name
    ...
    @content ||= @choices.map { |choice| option(value: choice[0], label: choice[1]) }.join.html_safe if @choices
  end
